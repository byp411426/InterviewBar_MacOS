...
1. 从上方 Releases 下载 `InterviewBar-v0.10.3-macOS-arm64.zip`，解压。
...
链接、公司、邮件正文和备注等输入框支持 Mac 标准快捷键：先点入输入框，使用 **⌘A 全选、⌘C 复制、⌘V 粘贴、⌘X 剪切、⌘Z 撤销、⇧⌘Z 重做**。粘贴或修改后仍需点保存才会更新记录。
...
8. **选择更新对象**：新邀请默认新增独立安排，同一家公司的笔试、一面、二面、三面分别保留，已完成记录不会被邮件导入覆盖。只有同一轮待办改期或补充信息时，才在“日程处理”中明确选择原记录；“投递表处理”仍用于更新公司投递汇总。
...
这些记录也有右侧 **…** 菜单，操作与普通安排一致：

- **编辑安排**：修正公司、岗位、类型、轮次、状态、日期、时刻和链接。未知信息可以留空。
- **标记已完成**：没有具体时间也能完成，从“待办”移到“已完成”，顶部数量同步更新。
- **取消安排 / 标记未通过**：记录保留在“全部”；可以通过 **恢复待办** 放回待确认列表。
- **删除记录**：核对确认提示后删除这条记录；不可撤销，投递总表、其他日程、原邮件和导入历史仍保留。

也可打开 **投递总表与日程 → 日程明细**，选中记录后使用右侧的 **编辑安排 / 更多操作**。筛选状态和导出的 CSV 都显示实际状态。

编辑时，选择“具体时间已知”，补齐公司、类型、日期与时刻后，保存会将同一条记录移入普通日程，不重复新增。仍缺少信息时保留记录，不设置定时提醒。“只有日期”和“从该日起陆续安排”不会自动补成午夜。

已完成但时间仍不明确的记录，可在“已完成”和秋招之旅的 **时间或信息待补记录** 中找到。它们暂不计入按周图表，也不受周范围筛选影响；补齐明确安排时间后再计入对应周。

操作会保存到本机，重启后保留；保存失败会提示并保留原记录。修改日程状态不会自动改写飞书或投递总表，也不会开放同一封邮件重复导入。
