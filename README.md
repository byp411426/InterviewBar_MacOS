...
1. 从上方 Releases 下载 `InterviewBar-v0.8.0-macOS-arm64.zip`，解压。
...
在菜单栏展开面板底部，点 **秋招之旅**，打开可调整大小的独立窗口。

- 顶部展示四种类型的累计已完成次数，不受下方筛选影响。
- 时间轴按**北京时间、周一至周日**分组，按最近一周在前排列；只列有记录的周。
- 每周列出公司、类型、岗位、安排时间与状态。下方类型筛选和“只看已完成”同时影响时间轴和该周数字。
- 周统计按安排的日期（开始日或截止日）归周，**不是实际完成时刻**。例如提前完成下周截止的测评，计在截止日所在周。
- 只有状态为“已完成”的安排计入次数。过期、取消、未通过以及没有明确日期时间的待通知记录不自动算作参加。
- 时间轴右侧状态菜单可以补标完成或恢复待办，累计数字会立即更新。关闭或重启后记录仍在。
- 120 条鼓励语在本机随机选择，点“换一句”不会产生 API 请求，也不会连续重复同一句。

次数用于回顾经历，不是成绩排名。应用没有连胜惩罚、强制打卡或把求职结果等同个人价值的评分。
