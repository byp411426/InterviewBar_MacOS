...
@main struct JourneyTests {
    @MainActor static func main() throws {
        ...
        let recent = JourneyStats.chartWeeks([sun, mon, pending, cancelled], now: date("2027-01-04 12:00"), range: .eight, metric: .completed)
        assert(recent.count == 8 && recent.last!.count(.aiInterview) == 1)
        assert(recent.dropLast().last!.count(.exam) == 1)
        assert(recent.dropLast(2).allSatisfy { $0.events.isEmpty })
        assert(recent.flatMap(\.events).count == 2)
        assert(JourneyStats.cumulative(recent, kind: .exam, through: 7) == 1)
        let all = JourneyStats.chartWeeks([sun, mon, pending, cancelled], now: date("2027-01-01 12:00"), range: .all, metric: .scheduled)
        assert(all.count == 2 && all.flatMap(\.events).count == 4)
        let empty = JourneyStats.chartWeeks([], now: date("2027-01-04 12:00"), range: .twelve, metric: .completed)
        assert(empty.count == 12 && empty.allSatisfy { $0.events.isEmpty })
        let snapshot = WidgetSnapshot.make(events: [sun, mon, pending, cancelled], now: date("2026-12-31 12:00"))
        assert(snapshot.next(at: date("2026-12-31 12:00"))?.id == pending.id)
        assert(snapshot.next(at: date("2027-01-01 10:00")) == nil)
        assert(snapshot.count("笔试", at: date("2027-01-03 23:59")) == 1)
        assert(snapshot.count("笔试", at: date("2027-01-04 00:00")) == 0)
        assert(snapshot.count("AI面试", at: date("2027-01-04 00:00")) == 1)
        let restored = try JSONDecoder().decode(WidgetSnapshot.self, from: JSONEncoder().encode(snapshot))
        assert(restored.upcoming.count == 1)
